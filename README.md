...
### 自动识别规则
- **前缀匹配**：监控"你的联系人"会自动包含"你的联系人小号"等（chat 名以监控名为前缀）
- **档案 alias 兜底**：如果档案 `.md` 里写了某个 wxid（即使 wechat-cli 缓存里没拉到），也会显示

### 手动管理
- 点头像或名字 → 弹窗顶部"多账号"区
- **+ 添加**：手动加一个未识别的微信账号（填 username、显示名、备注）
- **重命名（铅笔）**：给 wxid_xxx 这种"机器名"起个"人话名"（比如 `wxid_xxxxxxxxxxxxxxxx` → `your_wechat_id`），列表里会优先显示人话名
- **⭐ 设为主账号**：标哪个是主联系对象
...
**Q: 微信里看到的微信号是 `your_wechat_id`，但面板里显示 `wxid_xxxxxxxxxxxxxxxx`？**
...
- `config.yaml` 含你的 LLM API 密钥和档案目录绝对路径，**已加入 `.gitignore`**，不会泄露
- `state.json` 含手动联系人 / 账号 / label，也已 `.gitignore`
- `.auth_token` 是面板 API 鉴权用的 token，每次启动随机生成，`.gitignore`
- `monitor.log` 含消息摘要和 LLM 调用记录，`.gitignore`
- `*.app/`（macOS 启动器）也是 `.gitignore`

发布到 GitHub 之前请确认以上文件都已被排除。
